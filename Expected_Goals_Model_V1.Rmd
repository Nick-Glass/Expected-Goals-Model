...
[![NHL Rink Dimensions](Sport-Hockey-field-Template.png)](https://www.conceptdraw.com/How-To-Guide/ice-hockey-rink-dimensions)
...
![Harry Shomer Results](Harry%20Shomer%20XG%20Results.PNG)

![Patrick Bacon Results](Patrick%20Bacon%20XG%20Results.PNG)

![Evolving Hockey Results](Evolving%20Wild%20XG%20Results.PNG)
...
An alternate method to predict players goals could be using logistic regression to predict a "yes" or "no" binary outcome.

```{r,warning=FALSE,message=FALSE}
# create confusion matrix
# In-Sample 
confusionMatrix(as.factor(ifelse(pred_XG_full_train >= 0.5, "1", "0")), as.factor(XG_train$Is_Goal), 
                positive = "1")

# Out-of-Sample
confusionMatrix(as.factor(ifelse(pred_XG_full_valid >= 0.5, "1", "0")), as.factor(XG_valid$Is_Goal), 
                positive = "1")
...
costfunc <- function(obs, pred_p, pcut){
  weight1 <- 7 # define the weight for "true=1 but pred=0" (false negative)
  weight0 <- 1 # define the weight for "true=0 but pred=1" (false positive)
  c1 <- (obs == 1) & (pred_p < pcut) # count for "true=1 but pred=0" (false negative)
  c0 <- (obs == 0) & (pred_p >= pcut) # count for "true=0 but pred=1" (false positive)
  cost <- mean(weight1 * c1 + weight0 * c0) # misclassification with weight
  return(cost) # you have to return a value when you write R functions
} # end of the function

# define a sequence from 0.01 to 1 by 0.01
p_seq <- seq(from = 0.01, to = 1, by = 0.01)
...
for (i in 1:length(p_seq)) {
  cost[i] <- costfunc(obs = XG_train$Is_Goal, pred_p = pred_XG_full_train, pcut = p_seq[i])
} # end of the loop
...
optimal_XG_full_train <- p_seq[which(cost == min(cost))]
optimal_XG_full_train

# create confusion matrix
# In-Sample 
confusionMatrix(as.factor(ifelse(pred_XG_full_train >= optimal_XG_full_train, "1", "0")), as.factor(XG_train$Is_Goal), 
                positive = "1")
...
cost <- rep(0, length(p_seq))
for (i in 1:length(p_seq)) {
  cost[i] <- costfunc(obs = XG_valid$Is_Goal, pred_p = pred_XG_full_valid, pcut = p_seq[i])
} # end of the loop
...
## find optimal cut off
optimal_XG_full_valid <- p_seq[which(cost == min(cost))]
optimal_XG_full_valid

# Out-of-Sample
confusionMatrix(as.factor(ifelse(pred_XG_full_valid >= optimal_XG_full_valid, "1", "0")), as.factor(XG_valid$Is_Goal), 
                positive = "1")
```

## Analysis:

Using this method the model could be adjusted to include more correctly classified goals as goals by changing the threshold from 0.5 to the optimal cut off found in the analysis above. This would balance the sensitivity and specificity in order to have more predicted goals. In this method there would not be a probability of scoring rather a "yes" or "no" binary outcome. This model might be worth exploring in the future.
